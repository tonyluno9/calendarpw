...
export default function SpacesPage() {
  ...
  function startEdit(space) {
    ...
    setForm({
      ...
      available_from: String(space.available_from || "07:00").slice(0, 5),
      available_to: String(space.available_to || "22:00").slice(0, 5),
    });
  }
  ...
  async function remove(id) {
    ...
    setError("");
    try {
      const res = await apiFetch(`${BASE_URL}/spaces/${id}`, { method: "DELETE" });
      const data = await res.json().catch(() => ({}));
    ...
    <Layout>
      <header className="mb-6 px-2">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Crea, edita y elimina categorías (spaces).
        </p>
      </header>
